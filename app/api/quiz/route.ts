...
export async function GET(req: Request) {
  try {
    ...
    if (authedUserId) {
      ...
      const plan = String(prof?.plan ?? "trial");
    ...
    if (allowedCount <= 0) {
      return NextResponse.json({ questions: [], allowedCount: 0 });
    }
    ...
    const message =
      err instanceof Error ? err.message : "Unexpected server error in /api/quiz";
