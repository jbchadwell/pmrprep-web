...
import { ANON_PREVIEW_LIMIT, getAnonUsed, incrementAnonUsed } from "@/lib/trial";
...
mode: "quiz",
  ...
  // Auth snapshot for synchronous gating (avoids async race)
  const authRef = useRef<{ isAuthed: boolean; token?: string | null }>({ isAuthed: false, token: null });
  const [authWallOpen, setAuthWallOpen] = useState(false);
  const [authWallMsg, setAuthWallMsg] = useState<string>("");
  ...
  async function fetchNewQuiz() {
    ...
    if (mode === "custom") {
      ...
      res = await fetch("/api/quiz/custom", {
        ...
        body: JSON.stringify({
          count: Math.min(20, Number(config.count ?? 10)),
          primaryCategories: config.primaryCategories,
        }),
      });
    } else {
      const countParam = Number(params.get("count") ?? "10");
      const count = Number.isFinite(countParam) ? Math.min(50, Math.max(1, countParam)) : 10;
...
mode: "quiz",
    ...
    // Once a quiz is created, remove fresh=1 so returning after login restores localStorage state.
    try {
  ...
  useEffect(() => {
    ...
    // If a saved quiz exists, always restore it — even if URL contains fresh=1.
    // This prevents losing progress when returning from signup/login.
    const shouldBypassRestore = !saved ? (resume === "1" ? false : fresh === "1") : false;
    const savedToUse = shouldBypassRestore ? null : saved;

    if (savedToUse) {
      try {
        const parsed = JSON.parse(savedToUse) as QuizState;
  ...
  // Keep authRef in sync (no async needed inside submitAnswer)
  useEffect(() => {
    let mounted = true;
    supabaseBrowser.auth.getSession().then(({ data }) => {
      if (!mounted) return;
      authRef.current.isAuthed = Boolean(data.session);
      authRef.current.token = data.session?.access_token ?? null;
    });

    const { data: sub } = supabaseBrowser.auth.onAuthStateChange((_event, session) => {
      authRef.current.isAuthed = Boolean(session);
      authRef.current.token = session?.access_token ?? null;
    });
...
const hasAnswered = hasSubmitted;
  ...
  function submitAnswer() {
    ...
    // Strict anonymous preview gate: max 3 ANSWERED questions per device before requiring account.
    if (!authRef.current.isAuthed) {
      const used = getAnonUsed();
      if (used >= ANON_PREVIEW_LIMIT) {
        setAuthWallMsg(
          `Anonymous preview is limited to ${ANON_PREVIEW_LIMIT} answered questions. Create a free account to unlock your 20-question trial.`
        );
        try {
          localStorage.setItem("pmrprep_phase1_quiz_state_v2", JSON.stringify(state));
        } catch {}
        setAuthWallOpen(true);
        return;
      }
      // IMPORTANT: increment immediately so the gate is deterministic (no async race).
      incrementAnonUsed(1);
    }
  ...
  if (state.mode === "summary") {
    return (
      ...
      <>
{/* Trial ended CTA */}
      {trialStatus?.plan === "trial" && Number(trialStatus?.remaining ?? 0) <= 0 ? (
        <div className="mb-4 rounded-2xl border border-blue-200 bg-blue-50 p-4">
          <div className="text-sm font-semibold text-blue-900">Free trial complete</div>
          <div className="mt-1 text-sm text-blue-900/80">
            You’ve used your 20-question trial. Upgrade to keep studying and unlock unlimited questions.
          </div>
          <div className="mt-3">
            <button
              type="button"
      ...
      {authWallOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            <div className="text-lg font-semibold">Create an account to continue</div>
            <div className="mt-2 text-sm text-gray-700">{authWallMsg}</div>
            <div className="mt-5 flex gap-2">
              <button
                type="button"
                onClick={() => {
                  // Force-save state before navigating away (avoids losing Q1–Q3 after signup)
                  try {
                    localStorage.setItem("pmrprep_phase1_quiz_state_v2", JSON.stringify(state));
                  } catch {}
